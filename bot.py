...
# ---------- تنظیمات جدید ----------
TOKEN = "8839873832:AAGP0lnJcysost5fdtTTz18g_1Y0NWbLo04"
...
UNLIMITED_PRICES = {'single': 120000, 'double': 150000, 'triple': 200000, 'quad': 250000, 'penta': 290000}
PRICE_PER_GB = 6000
...
def init_db():
    ...
    if c.fetchone()[0] == 0:
        ...
        # نامحدود
        default_prices.append(('unlimited_single', 120000, 'پنل نامحدود تک‌کاربره'))
        default_prices.append(('unlimited_double', 150000, 'پنل نامحدود دو‌کاربره'))
        default_prices.append(('unlimited_triple', 200000, 'پنل نامحدود سه‌کاربره'))
        default_prices.append(('unlimited_quad', 250000, 'پنل نامحدود چهار‌کاربره'))
        default_prices.append(('unlimited_penta', 290000, 'پنل نامحدود پنج‌کاربره'))
        for key, value, desc in default_prices:
            c.execute("INSERT INTO prices (key, value, description) VALUES (?,?,?)", (key, value, desc))
        conn.commit()
    else:
        # migration: قیمت پلن‌های نامحدود به تعرفه‌ی جدید به‌روزرسانی می‌شود
        # و پلن‌های چهار و پنج‌کاربره در صورت نبودن اضافه می‌شوند
        migrated_unlimited_prices = {
            'unlimited_single': (120000, 'پنل نامحدود تک‌کاربره'),
            'unlimited_double': (150000, 'پنل نامحدود دو‌کاربره'),
            'unlimited_triple': (200000, 'پنل نامحدود سه‌کاربره'),
        }
        new_unlimited_prices = {
            'unlimited_quad': (250000, 'پنل نامحدود چهار‌کاربره'),
            'unlimited_penta': (290000, 'پنل نامحدود پنج‌کاربره'),
        }
        for key, (value, desc) in migrated_unlimited_prices.items():
            c.execute("UPDATE prices SET value = ? WHERE key = ?", (value, key))
        for key, (value, desc) in new_unlimited_prices.items():
            c.execute("INSERT OR IGNORE INTO prices (key, value, description) VALUES (?,?,?)", (key, value, desc))
        conn.commit()
...
def unlimited_plan_name(period):
    names = {
        'single': 'تک‌کاربره',
        'double': 'دو‌کاربره',
        'triple': 'سه‌کاربره',
...
def load_prices_from_db():
    ...
    UNLIMITED_PRICES['single'] = price_dict.get('unlimited_single', 120000)
    UNLIMITED_PRICES['double'] = price_dict.get('unlimited_double', 150000)
    UNLIMITED_PRICES['triple'] = price_dict.get('unlimited_triple', 200000)
    UNLIMITED_PRICES['quad'] = price_dict.get('unlimited_quad', 250000)
    UNLIMITED_PRICES['penta'] = price_dict.get('unlimited_penta', 290000)
...
def unlimited_plans_keyboard():
    keyboard = InlineKeyboardMarkup(row_width=2)
    keyboard.add(
        InlineKeyboardButton("👤 تک‌کاربره\n{:,} ت".format(UNLIMITED_PRICES['single']), callback_data="buy_unlimited_single", style='primary'),
        InlineKeyboardButton("👥 دو‌کاربره\n{:,} ت".format(UNLIMITED_PRICES['double']), callback_data="buy_unlimited_double", style='primary'),
        InlineKeyboardButton("👥 سه‌کاربره\n{:,} ت".format(UNLIMITED_PRICES['triple']), callback_data="buy_unlimited_triple", style='primary'),
        InlineKeyboardButton("👥 چهار‌کاربره\n{:,} ت".format(UNLIMITED_PRICES['quad']), callback_data="buy_unlimited_quad", style='primary'),
        InlineKeyboardButton("👥 پنج‌کاربره\n{:,} ت".format(UNLIMITED_PRICES['penta']), callback_data="buy_unlimited_penta", style='primary')
    )
    keyboard.add(InlineKeyboardButton("🔙 بازگشت", callback_data="back_main", style='danger'))
    return keyboard
...
def pending_orders_keyboard(orders, order_type='order'):
    ...
    for order in orders:
        ...
        volume = order[2]
        price = order[3]
        final_price = order[18] if len(order) > 18 and order[18] else price
        tracking = order[19] if len(order) > 19 else "-----"
        if order_type == 'test':
            label = f"🧪 تست - {tracking} - کاربر {user_id}"
        else:
            if len(order) > 12 and order[14] == 1:
                period = order[15]
                plan = unlimited_plan_name(period)
...
def admin_prices_keyboard():
    keyboard = InlineKeyboardMarkup(row_width=1)
    keyboard.add(InlineKeyboardButton("♾️ قیمت نامحدود", callback_data="admin_prices_unlimited", style='primary'))
    keyboard.add(InlineKeyboardButton("🔙 بازگشت", callback_data="admin_back_main", style='danger'))
    return keyboard
...
def admin_unlimited_prices_keyboard():
    keyboard = InlineKeyboardMarkup(row_width=1)
    for period in ['single', 'double', 'triple', 'quad', 'penta']:
        keyboard.add(InlineKeyboardButton(
            f"{unlimited_plan_name(period)}: {UNLIMITED_PRICES[period]:,} تومان",
            callback_data=f"admin_price_edit_unlimited_{period}", style='primary'))
    keyboard.add(InlineKeyboardButton("🔙 بازگشت", callback_data="admin_prices", style='danger'))
    return keyboard
...
def generate_invoice_text(order):
    ...
    price = order[3]
    final_price = order[18] if len(order) > 18 and order[18] else price
    discount_amount = order[17] if len(order) > 17 and order[17] else 0
    tracking = order[19] if len(order) > 19 else ""
    is_unlimited = order[14] if len(order) > 14 else 0
    unlimited_period = order[15] if len(order) > 13 else None

    if is_unlimited:
        plan_name = unlimited_plan_name(unlimited_period)
...
def show_pricing(user_id):
    ...
    text += "♾️ <b>پنل نامحدود (حجم و کاربر نامحدود):</b>\n"
    for period in ['single', 'double', 'triple', 'quad', 'penta']:
        text += f"📦 {unlimited_plan_name(period)} ➖ {UNLIMITED_PRICES[period]:,} تومان\n"
...
def callback_handler(call):
    ...
    if data.startswith("pay_wallet_"):
        ...
        if not order:
            bot.answer_callback_query(call.id, "❌ سفارش یافت نشد!", show_alert=True)
            return
        final_price = order[18] if len(order) > 18 and order[18] else order[3]
        balance = get_user_balance(user_id)
        if balance >= final_price:
            ...
            if len(order) > 12 and order[14] == 1:
                period = order[15]
                plan_name = unlimited_plan_name(period)
    ...
    if data == "wallet_history":
        ...
        for order in orders[:20]:
            ...
            if len(order) > 12 and order[14] == 1:
                period = order[15]
                plan = f"نامحدود ({unlimited_plan_name(period)})"
            else:
                plan = f"{order[2]} GB"
            final_price = order[18] if len(order) > 18 and order[18] else order[3]
    ...
    if data.startswith("admin_price_edit_unlimited_"):
        ...
        period_name = unlimited_plan_name(period)
...
def show_order_detail_for_admin(chat_id, order_id, is_test, edit_mode=False):
    ...
    final_price = order[18] if len(order) > 18 and order[18] else order[3]
    tracking = order[19] if len(order) > 19 else "ندارد"
    text = (f"{label} #{order_id}\n"
            ...
            f"⚡ وضعیت: {status_text}")
    if len(order) > 11 and order[11] == 1:
        text += f"\n📛 نام اکانت: {order[12]}"
    if order[6]:
